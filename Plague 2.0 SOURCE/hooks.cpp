...
// TODO: put in another file, and rename to something better
		...
		if (object && func) {
			...
			do {
				...
				if (Settings.FastReload)
				{
					...
					if (Util::SpoofCall(GetAsyncKeyState, 0x10))
					{
	...
	float* CalculateShotHook(PVOID arg0, PVOID arg1, PVOID arg2, UObject* object, UObject* func) {
		...
		{
			auto mesh = ReadPointer(hooks::TargetPawn, 0x280); //Offsets::Engine::Character::Mesh
			if (!mesh) return ret;
			//Offsets::Engine::StaticMeshComponent::ComponentToWorld
			auto bones = ReadPointer(mesh, 0x480);
			if (!bones) bones = ReadPointer(mesh, 0x480 + 0x10);
			if (!bones) return ret;

			float compMatrix[4][4] = { 0 };
			Util::ToMatrixWithScale(reinterpret_cast<float*>(reinterpret_cast<PBYTE>(mesh) + 0x1C0), compMatrix); //0x1C0
